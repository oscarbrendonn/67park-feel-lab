import {PETS,petSelection} from './pet-state.js?v=pets-1';
  ...
  return function PetInventory(){
    ...
    return h('section',{className:'park-pet-inventory','aria-label':'Pet companions'},
      ...
      h('div',{className:'park-pet-choices'},PETS.map(p=>h('button',{key:p.id,type:'button','aria-label':'Choose '+p.label,'aria-pressed':selected===p.id,onClick:()=>petSelection.select(p.id),style:{'--pet-accent':p.accent}},
        h('img',{src:new URL('../../pets/'+p.id+'.png',import.meta.url).href,alt:'',width:92,height:85}),h('strong',null,p.name),h('small',null,selected===p.id?'Following you':p.label)))),
      h('div',{className:'park-pet-tools'},h('button',{type:'button',disabled:!selected,onClick:()=>petSelection.select('')},'No pet'),h('a',{href:new URL('../../pets/',import.meta.url).href,target:'_blank',rel:'noopener'},'View in 3D ↗')),
