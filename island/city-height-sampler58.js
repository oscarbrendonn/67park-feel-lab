...
export function createCityHeightSampler58(meshes,{cellSize=3,minHeight=-Infinity,precision='float64'}={}){
 ...
 const bins=cells.map(v=>Uint32Array.from(v));let lastX=NaN,lastZ=NaN,lastCeiling=NaN,last=null;
 ...
 function sample(x,z,ceiling=Infinity){
  if(x===lastX&&z===lastZ&&ceiling===lastCeiling)return last;lastX=x;lastZ=z;lastCeiling=ceiling;last=null;
  ...
  for(const k of bins[gz(z)*cols+gx(x)]){
   ...
   if(height<=ceiling+1e-6&&height>y){y=height;last={object:meshes[ids[k/10]],point:{x,y,z}};}
  }
  return last;
 }
 return {sample,height:(x,z,ceiling=Infinity)=>sample(x,z,ceiling)?.point.y??null,stats};
}
