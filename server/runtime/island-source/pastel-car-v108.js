...
export const createPastelCar108=createCandyVehicle110;
...
export function installCarControls108({traffic,controller,isGame,canBoard=()=>true,data={},onExit=()=>{},headless=false,owner='local-car-'+(++carClientSequence110)}){
 ...
 if(!headless){
  ...
  hud.id='parkCarHUD108';hud.hidden=true;hud.innerHTML='<div class="car-readout"><strong>Pastel araba</strong><span id="carSpeed108">0 km/sa</span><small>WASD / oklar · Boşluk: fren · E: bin / in</small><span id="carMessage108" role="status" aria-live="polite"></span></div><div class="car-pad car-steer"><button data-drive="left" aria-label="Sola dön">◀</button><button data-drive="right" aria-label="Sağa dön">▶</button></div><div class="car-pad car-pedals"><button data-drive="reverse" aria-label="Geri git">Geri</button><button data-drive="brake" aria-label="Fren">Fren</button><button data-drive="gas" aria-label="Gaz">Gaz</button></div>';
 ...
 function refresh(){
  ...
  if(riding){
   ...
   hud.querySelector('#carSpeed108').textContent=Math.round(Math.abs(active.car.physics.speed)*3.6)+' km/sa · '+status.occupied.length+'/'+status.capacity+' koltuk';
 ...
 function drive(dt,input={}){
  ...
  if(p.reason)notify(p.reason==='road-edge'?'Yol sınırı · geri manevra yap.':'Önün dolu · geri manevra yap.');
